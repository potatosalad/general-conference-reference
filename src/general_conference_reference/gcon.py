...
OPENAI_API_CLIENT: AsyncOpenAI = AsyncOpenAI(api_key=OPENAI_API_KEY)
...
async def retry_on_rate_limit(
    ...
    *args,
    max_retries: int = 10,
    max_wait: float = 60.0,  # Maximum wait time in seconds
    jitter: float = 1.0,  # Random jitter factor
    **kwargs,
) -> Any:
    for attempt in range(max_retries):
        try:
            ...
            # Add jitter
            wait_time *= 1 + random.uniform(0, jitter)
...
def extract_wait_time(error_message: str) -> Optional[int]:
    """Extract wait time in milliseconds from OpenAI error message."""
    ...
    async def execute(self) -> None:
        ...
        if not os.path.isfile(markdown_file):
            ...
            cmd2 = [
                ...
                r"s#\[<sup>[0-9]\+</sup>\]([^)]*)##g",
                r"-e",
                r"s#<a [^>]*><sup>[0-9]\+</sup></a>##g",
                r"-e",
                r"s#\](/#](https://www.churchofjesuschrist.org/#g",
                r"-e",
                r's#href="/#href="https://www.churchofjesuschrist.org/#g',
            ]
            read, write = os.pipe()
    ...
    async def give_openai_more_money(self) -> None:
        ...
        if not os.path.isfile(outline_file):
            ...
            with open(self.markdown_file, "r") as f:
                text = f.read()
            chat_completion = await retry_on_rate_limit(
...
class GeneralConference:
    ...
    async def execute(self) -> None:
        ...
        # Fast: all at once (rate limit exceeded)
        await asyncio.gather(*[talk.give_openai_more_money() for talk in self.talks])

        # Fast (limited): only allow `max_concurrent` at a time
        # async def process_talks_limited(talks: list["Talk"], max_concurrent: int = 4) -> None:
        #     queue: deque["Talk"] = deque(talks)

        #     async def worker() -> None:
        #         while queue:
        #             talk: "Talk" = queue.popleft()
        #             await talk.give_openai_more_money()

        #     async with asyncio.TaskGroup() as tg:
        #         [tg.create_task(worker()) for _ in range(min(max_concurrent, len(talks)))]

        # await process_talks_limited(talks = self.talks, max_concurrent = 4)
    ...
    def root_dir(self) -> str:
        root_dir: str = f"{self.output_path}/{self.year}{self.month}"
